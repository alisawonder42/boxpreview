...
export function attachDebugMenu(options: DebugOptions) {
  ...
  if (lens) {
    ...
    folder.add(lens, 'cellSize', 6, 16, 0.5).name('cell size')
    folder.add(lens, 'depthEdge', 0, 4, 0.01).name('depth edge strength')
    folder.add(lens, 'colorEdge', 0, 2, 0.01).name('color edge strength')
    folder.add(lens, 'depthCoarse', 0, 2, 0.01).name('depth coarse')
    folder.add(lens, 'depthFine', 0, 2, 0.01).name('depth fine')
    folder.add(lens, 'surfaceDensity', 0, 1, 0.01).name('surface density')
    folder.add(lens, 'backgroundDensity', 0, 0.4, 0.01).name('background density')
    folder.add(lens, 'textureInfluence', 0, 1, 0.01).name('texture influence')
    folder.add(lens, 'vectorLength', 0.35, 1, 0.01).name('vector length')
    folder.add(lens, 'markBrightness', 0.2, 2, 0.01).name('vector brightness')
    folder.add(lens, 'glitch', 0, 0.1, 0.001).name('instability')
